...
import { Plus, Trash2, FileText, Calendar, Wallet, ShoppingBag, Hash } from 'lucide-react';
import {
  Table,
  TableBody,
  TableCell,
...
export default function TotalInvoicesPage() {
  ...
  const handleAdd = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    addGeneralInvoice({
      date: dateVal,
      day: dayName,
      invoiceCount: parseInt(formData.get('invoiceCount') as string) || 0,
  ...
  const totalInvoiceCounts = generalInvoices.reduce((acc, inv) => acc + (inv.invoiceCount || 0), 0);
    ...
    <div className="space-y-6 text-right">
      <div className="flex justify-between items-end flex-row-reverse">
        ...
        <div className="flex gap-4">
          <div className="bg-[#336699]/10 p-4 rounded-xl border border-[#336699]/20 text-left">
            <p className="text-[10px] text-[#336699] font-black uppercase tracking-wider mb-1">إجمالي عدد الفواتير</p>
            <h2 className="text-2xl font-black text-[#336699]">{totalInvoiceCounts.toLocaleString()}</h2>
          </div>
            ...
            <form onSubmit={handleAdd} className="space-y-4 text-right">
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <Input name="invoiceCount" type="number" placeholder="أدخل العدد" className="text-right pr-10" />
                </div>
              </div>
              <div className="space-y-2">
                  ...
                  generalInvoices.map((inv) => (
                    <TableRow key={inv.id} className="hover:bg-slate-50/50 transition-colors">
                      ...
                      <TableCell className="text-center">
                         <span className="font-black text-lg text-slate-700 bg-slate-100 px-3 py-1 rounded-lg">
                           {inv.invoiceCount || 0}
                         </span>
                      </TableCell>
